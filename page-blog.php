...
.blog-archive-card:hover {
    ...
    box-shadow: 0 16px 40px rgba(0,0,0,0.12), 0 20px 40px rgba(30,60,114,0.1);
}
/* Card Image */
...
.blog-archive-card:hover .blog-archive-card-link {
    color: #ffd700;
}
/* Navigation */
.blog-archive-nav {
...
.blog-archive-nav-pages .page-numbers {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    width: 42px;
...
.blog-archive-nav-pages .page-numbers.current {
    background: linear-gradient(135deg, #1e3c72 0%, #2a5298 100%);
    color: #ffffff;
    border-color: transparent;
}
.blog-archive-nav-pages .page-numbers:hover:not(.current) {
    background: #ffd700;
    color: #1e3c72;
    transform: translateY(-2px);
    border-color: transparent;
...
@media (max-width: 1024px) {
    .blog-archive-grid { grid-template-columns: repeat(2, 1fr); gap: 22px; }
}
@media (max-width: 640px) {
    .blog-archive-header h1 { font-size: 1.9em; }
    .blog-archive-grid { grid-template-columns: 1fr; gap: 20px; }
    .blog-archive-nav { flex-direction: column; }
    .blog-archive-nav-btn { width: 100%; text-align: center; }
}
</style>
